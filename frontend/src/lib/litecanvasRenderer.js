...
export class LabRenderer {
   ...
   * @param {(ctx: CanvasRenderingContext2D) => void} drawFn
   */
  ...
  drawSprite(imageKey, sx, sy, sw, sh, dx, dy, dw = sw, dh = sh) {
    ...
    this.ctx.drawImage(this.images[imageKey], sx, sy, sw, sh, dx, dy, dw, dh);
  }
...
export class BreadcrumbStage {
  ...
  draw(renderer) {
    ...
    if (this.index > 0) {
      ...
      ctx.strokeStyle = this.status === "completed" ? "#10b981" : "#374151";
      ctx.lineWidth = 3;
      ctx.beginPath();
      ctx.moveTo(prevX + 20, this.y);
