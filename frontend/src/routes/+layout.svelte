    ...
    <aside class="w-20 glass-card border-r border-border flex flex-col items-center py-6 gap-6">
      ...
      {#if $isAuthenticated}
        <div class="flex flex-col items-center gap-2">
          <div class="w-10 h-10 rounded-full bg-primary/20 flex items-center justify-center" title={$currentUser?.email}>
            <User class="w-5 h-5 text-primary" />
          </div>
          <button 
            onclick={handleSignOut}
            class="w-10 h-10 rounded-xl flex items-center justify-center text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-colors"
            title="Sign out"
          >
      ...
      {:else if !authLoading}
        <a 
          href="/login"
          class="w-10 h-10 rounded-xl flex items-center justify-center text-muted-foreground hover:text-primary hover:bg-primary/10 transition-colors"
          title="Sign in"
        >
          <User class="w-5 h-5" />
        </a>
