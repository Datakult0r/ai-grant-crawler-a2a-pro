<script lang="ts">
  import { Search, Filter, TrendingUp, Clock, Target, Award } from 'lucide-svelte';
...
<div class="p-8 space-y-6">
      ...
      <CardContent class="p-6">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-3xl font-bold text-secondary">{stats.deadlinesWeek}</p>
          </div>
  ...
  <Card class="glass-card border-accent/30">
    <CardContent class="p-4">
      <div class="flex items-center gap-4">
        <div class="w-3 h-3 rounded-full bg-accent animate-pulse-glow neon-glow-green"></div>
        <span class="text-sm text-foreground">Live crawler active - Last updated 2 minutes ago</span>
        <div class="flex-1"></div>
        <div class="flex gap-3 text-xs">
          <span class="text-muted-foreground">ANI <span class="text-accent">●</span></span>
          <span class="text-muted-foreground">IAPMEI <span class="text-accent">●</span></span>
          <span class="text-muted-foreground">COMPETE <span class="text-accent">●</span></span>
          <span class="text-muted-foreground">Horizon EU <span class="text-accent">●</span></span>
        </div>
      </div>
    </CardContent>
  </Card>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
    {#each grants as grant}
      <Card class="glass-card hover:border-primary/50 transition-all duration-300 group">
        <CardHeader>
          <div class="flex items-start justify-between gap-4">
            <div class="space-y-2 flex-1">
              ...
              <div class="flex gap-2">
                ...
                <Badge variant="outline" class="border-secondary/30 text-secondary">
                  {grant.region}
                </Badge>
              </div>
            </div>
            <Badge class="bg-accent/20 text-accent border-accent/30 text-lg px-3 py-1">
              {grant.amount}
            </Badge>
          </div>
        </CardHeader>
        <CardContent class="space-y-4">
          ...
          <div class="flex items-center justify-between">
            ...
            <span class="text-sm font-medium {grant.daysLeft < 90 ? 'text-destructive' : 'text-accent'}">
              {grant.daysLeft} days left
            </span>
          </div>

          <!-- Action Button -->
          <Button class="w-full bg-gradient-to-r from-primary to-secondary hover:opacity-90 neon-glow-purple">
            Apply Now
          </Button>
        </CardContent>
      </Card>
